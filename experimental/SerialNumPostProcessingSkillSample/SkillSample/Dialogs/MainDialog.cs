...
namespace SkillSample.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        private readonly SerialNumPostProcessAction _sampleAction;
            ...
            : base(nameof(MainDialog))
        {
            ...
            _sampleAction = serviceProvider.GetService<SerialNumPostProcessAction>();
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else if (activity.Type == ActivityTypes.Event)
            {
                ...
                if (!string.IsNullOrEmpty(ev.Name))
                {
                    switch (ev.Name)
                    {
                        case "SerialNumPostProcessAction":
                            {
                                PreProcessedSerialNumInput actionData = null;

                                if (ev.Value is JObject eventValue)
                                {
                                    actionData = eventValue.ToObject<PreProcessedSerialNumInput>();
                                }

                                // Invoke the SampleAction dialog passing input data if available
                                return await stepContext.BeginDialogAsync(nameof(SerialNumPostProcessAction), actionData, cancellationToken);
                            }
